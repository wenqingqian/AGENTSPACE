...
5. **Proactively ask three questions** (may use AskUserQuestion to collect all at once; for any unanswered, leave placeholder comments — never fabricate):
   ...
   3. **key code repositories**: workspaces often contain multiple repos; ask the user to identify project-critical repos and existing relevant code files within them (these are the objects for deep analysis in the next step)
