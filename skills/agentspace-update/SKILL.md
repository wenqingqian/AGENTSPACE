...
If an intermediate version's archive is missing (versions folded into one archive — e.g. the v0.4.1 CHANGELOG declares "Upgrade from v0.4.0" and covers every marker in the v0.4.1-v0.4.4 range), skip to the next existing archive: the merged CHANGELOG covers the gap and a workspace marked at a merged-away version upgrades by the merged archive's instructions.
