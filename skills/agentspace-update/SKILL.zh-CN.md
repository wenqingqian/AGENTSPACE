...
中间版本的归档缺失时（版本被合并进单个归档——例如 v0.4.1 的 CHANGELOG 声明 "Upgrade from v0.4.0" 并覆盖 v0.4.1-v0.4.4 区间内的全部标记），跳过缺失的中间档案，直接读下一个存在的归档：合并版 CHANGELOG 覆盖该缺口，标记落在被合并版本上的工作区按合并归档的指令升级。
...
- 用户规则节逐字保留 — 与 项目简介/根仓库简介 同级：该节归用户所有，更新绝不改写、重排或删减其中条目（只有用户能在其中创建/修改/删除规则）；托管节的既有规则保持不变（结构树与模块清单仍更新到目标架构，纪律仍按 changelog 文本操作更新）
