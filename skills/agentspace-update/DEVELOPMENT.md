...
7. **Never write a realized canonical bookkeeping id into tracked non-ledger files** — the plugin repo is a registered key repo under its own valveless gate: a realized `plan:0NNN` / `iteration_0NNN` literal in tests, docs, or skills would block every future edit of that line at the commit gate and be reported by doctor [15] forever. Prose uses the `plan:NNNN` placeholder form (or the regex itself); test fixtures construct ids at runtime (`printf 'plan:%04d' 13`); verify-release [12] guards the tree at the release gate.
