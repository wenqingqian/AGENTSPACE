...
1. MUST 运行硬脚本: `bash AGENTSPACE/scripts/status.sh <插件版本>`。脚本输出全部机械节, 软槽以固定占位符呈现: `- 项目: —`、`- 近期主线: —`, 以及每个宿主 commit 一行 `  概括[<sha>]: —`。
2. MUST 从脚本输出收集宿主 commit SHA 列表: 匹配 `^  概括\[([0-9a-f]+)\]: —` 的每一行。
...
- 近期动态 = 机械活动时间线: 索引日期列的工作区事件(计划创建/完成、迭代开启/关闭、笔记新增、交接生成) + 宿主仓库 commit(iteration 对应的真正代码改动, 带每 commit stat 与 iteration 关联) + 工作区自身台账 commit(类型前缀映射中文, 如 plan:→计划 / fix:→修复)。
