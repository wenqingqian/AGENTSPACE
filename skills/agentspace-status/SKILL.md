...
1. MUST run the hard script: `bash AGENTSPACE/scripts/status.sh <plugin-version>`. The script prints every mechanical section, including the soft slots as fixed placeholders: `- 项目: —`, `- 近期主线: —`, and one `  概括[<sha>]: —` line per host commit.
2. MUST collect the host commit SHA list from the script output: every line matching `^  概括\[([0-9a-f]+)\]: —`.
...
> 3. For EACH SHA in this list: <SHA1> <SHA2> — inspect it via `git -C AGENTSPACE/.. rev-parse --show-toplevel` to locate the host repo (skip if not a git repo), then `git -C <host-root> show --stat <sha>` and `git -C <host-root> log -1 --format='%s' <sha>`; write `COMMIT_SUMMARY_<sha>=<2-3 句中文>` — what the change does and why, linking to its iteration/plan when the workspace records it. If a SHA cannot be inspected, omit its line.
...
- 近期动态 is a mechanical activity timeline: workspace events from the index date columns (plan created/completed, iteration opened/closed, note added, handoff produced), host-repo commits (the code the iterations correspond to — with per-commit stats and iteration linkage), and the workspace's own ledger commits (type prefix mapped to a Chinese label, e.g. plan:→计划 / fix:→修复).
