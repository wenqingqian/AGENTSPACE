...
- **基准计划是不可变方向锚点**: 跨多个 plan 的方向在 `plan/base/` 锚定一次(独立 base:NNNN 计数); 派生 plan 经 `--base NNNN` 声明归属; 用户审核后文件按校验和冻结, 偏离锚点成为必须交用户决断的显性事件
...
└── AGENTSPACE/                # 独立 git 仓库
    ...
    ├── plan.md                # 入口: Todo + Done(最近10条) + Base 基准计划
