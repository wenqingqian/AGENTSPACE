...
- **Code hygiene (built-in)**: code, comments, and commit text in registered repos follow the agentspace-code-clean rules by default (comment MUSTs: no feedback-driven or context-missing residue, no redundancy, no test-instance citations, no process narration, no machine fingerprints or secrets; commit title/body rules); cleaning up past code or history runs only on the user's explicit request, per the skill's CLEANUP procedures
